$.widget('aerolab.blockrain', {
    ...
    options: {
      ...
      autoBlockSize: 50, // The max size of a block for autowidth mode
      ...
      restartButtonText: 'Start New Round',
      scoreText: 'Score',
     ...
     */
    start: function() {
      this._filled.numLives = 3;
      this._$livesText.text(this._filled.numLives);
    ...
    restart: function() {
      //this._board.cur = this._popQueue();
      //console.log("restarting, removing first element");
      //this.traceQueue();
      //$('.con-inner img').first().remove();
    ...
    gameover: function() {
      ...
      //console.log("Game Over for realzies");
    ...
    updateSizes: function() {
      ...
      this._PIXEL_HEIGHT = this.element.innerHeight() - this.element.innerHeight()/10;
    ...
    _createQueue: function(queue) {
      console.log("Creating queue");
      for (x=0; x<10; x++) {
        var nextBlock = this._board.nextShape();
        this.blockQueue.push(nextBlock);
        // Create Img
        this._addImg(nextBlock);
      }
    },

    _addImg: function(_shape) {
      //console.log("adding image");
        var shapeType = eval(_shape).blockType;
        var imgIndex = 0;
        ...
        switch(shapeType) {
          ...
          case "voucher":
            imgIndex = Math.round(Math.random()*7);
            break
        }

        if (imgIndex!=0) {
          var markup = "<img src='./products/product"+imgIndex+".png' align='bottom'>"
          $('.con-inner').append(markup);
        }
    ...
    _popQueue: function() {
      ...
      this.blockQueue.push(nextBlock);
      this._addImg(nextBlock);
      $('.con-inner img').first().fadeOut(300, function() {this.remove()});
      return this.blockQueue.shift();
    },

    traceQueue: function() {
      $(this.blockQueue).each(function(a,e){
    ...
    _SetupShapeFactory: function(){
      ...
      this._shapeFactory = {
        ...
        voucher: function() {
          var ver = [-4,-2,-3,-2,-2,-2,-1, -2, 0, -2, 1, -2, 2, -2,3,-2,4,-2,5,-2],
          hor = [-4,-2,-3,-2,-2,-2,-1, -2, 0, -2, 1, -2, 2, -2,3,-2,4,-2,5,-2];
          return new Shape(game, [ver, hor, ver, hor], true, 'voucher');
        }
      };
    },
    ...
    _SetupBoard: function() {
      ...
      this._board = {
        ...
        nextQueueShape: function() {
          return game._popQueue();
        },

        nextShape: function(_set_next_only) {
          var makeVoucher = false;
          game.options.blockCount++;

          if (voucher == true && game.options.blockCount == game.options.voucherRandom) {
            makeVoucher = true;
          }
          ...
          } else if (makeVoucher==true) {
            func = game._randomShapes(true);
          } else {
            func = game._randomShapes(false);
          }
        ...
        animate: function() {
          ...
          if( !this.paused && !this.gameover ) {
            ...
            if (this.cur == null) this.cur = game._popQueue();
          ...
          if( gameOver ) {
            ...
            //game.gameover();
            game._filled.numLives --;
            game.loseLife();

            if (game._filled.numLives == 0) {
              game.gameover();
            } else {
            //  $('.con-inner img').first().remove();
              game.restart();
    ...
    _randChoice: function(choices) { return choices[this._randInt(0, choices.length-2)]; },
    ...
    _createUI: function() {
      ...
      // Score
      game._$scoreText = $(document).find('.blockrain-score-num');
      game._$livesText = $(document).find('.level .button-content');
    ...
    _randomShapes: function(_voucher) {
      // Todo: The shapefuncs should be cached.
      var shapeFuncs = [];
      $.each(this._shapeFactory, function(k,v) { shapeFuncs.push(v); });
      if (_voucher) {
        var _newShape = shapeFuncs[7];
      } else {
        var _newShape = this._randChoice(shapeFuncs);
      }
      return _newShape;
    },
      ...
      var handleKeyPress = function(evt) {
        ...
        if (game._board.cur) {
          ...
          switch(evt.keyCode) {
            ...
            case 32: /*space*/  game._board.cur.rotate(true); break;
      ...
      if( ! game.options.autoplay ) {
        if( enable ) {
          ...
          $(document).on('touchstart', function(e) {
            ...
            if (game.options.isPlaying) {
              
            switch ($(e.target).parent()[0].className) {
              case "go-left":
                e.preventDefault();
